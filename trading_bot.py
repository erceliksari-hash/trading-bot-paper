...
if os.path.exists("wallet.json"):
    try:
        ...
        logger.exception("wallet.json exists but could not be read, initializing default wallet")
        WALLET = {"USD": 10000.0}
else:
    WALLET = {"USD": 10000.0}
...
def send_telegram(text, image_path=None, retries=3, backoff=2):
    ...
    # Escape text for HTML to avoid formatting issues / injection
    safe_text = html.escape(str(text))

    for attempt in range(1, retries + 1):
        try:
            url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
            payload = {"chat_id": TELEGRAM_CHAT_ID, "text": safe_text, "parse_mode": "HTML"}
...
def fetch_ohlcv_ccxt(symbol, timeframe="15m", limit=200):
    ...
    try:
        ...
        df = df.dropna(subset=["close"])  # ensure we have close prices
        return df
    except Exception as e:
...
def fetch_ohlcv_yfinance(ticker, period="7d", interval="15m"):
    ...
    try:
        ...
        # Ensure necessary columns exist; if missing create and fill with NaN
...
def position_size(balance_usd, risk_pct, price, atr):
    try:
        ...
        # Safety cap: don't allow astronomically large positions
        max_qty = CONFIG.get("max_position_qty", None)
...
def paper_execute(symbol, meta, side):
    ...
    if side == "buy":
        ...
        if cost > balance:
            qty = (balance / price) if price and price > 0 else 0.0
            cost = qty * price
        if qty <= 0:
        ...
        if meta and meta.get("signal"):
            if CONFIG.get("paper_trade", True):
                ...
                try:
                    ...
                    logger.exception("Failed to send telegram for signal")
        else:
